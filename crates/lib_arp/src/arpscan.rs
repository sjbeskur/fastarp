...
pub fn scan_v4(iface_name: &str) -> ArpResult<HashMap<String, ArpNode>> {
    ...
    while let Ok(arp) = receiver.try_recv() {
        ...
        let n = ArpNode {
            mac_address: arp.1.to_string(),
            ipv4_address: arp.0.to_string(),
            ipv4_target: arp.2.to_string(),
            ping_ms: t,
        };
...
pub(crate) fn compute_chunk_size(list_size: usize) -> usize {
    ...
    if list_size > 65536 {
        warn!("this network is too big to support and we should Panic");
        return 410;
    }
    ...
    let rslt = onedot5.powi(distance) * 16.0;  // 16 is arbitrary coefficient
...
fn send_arp(iface: NetworkInterface, source_ip: IpAddr, ip_list: &[Ipv4Addr]) -> HashMap<Ipv4Addr, SystemTime> {
    ...
    for &target_ip in ip_list.iter() {
        match send_arp_packet(&mut tx, source_mac, source_v4, target_mac, target_ip) {
    ...
    let h = thread::spawn(move || {
        ...
        while let Ok(data) = rx.next() {
            ...
            if arp_packet.get_operation() == arp_reply_op {
                let result: (Ipv4Addr, MacAddr, Ipv4Addr, SystemTime) = (
...
fn send_arp_packet(
    ...
    target_mac: MacAddr,
    target_ip: Ipv4Addr,
) -> ArpResult<(Ipv4Addr, SystemTime)> {
    trace!("sending arp packet to {} - {} ", target_ip, source_mac);
